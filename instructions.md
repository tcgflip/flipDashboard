...
- **`targetPrice` is a vendor-table negotiation goal, not a verified online listing (fixed 2026-09-15 after Ryan couldn't find a Pikachu VMAX PSA 10 anywhere near the $58 target listed)** — this routine has no way to confirm a specific online listing exists at a given price, and inventing an aggressive discount below market to make ROI look good produced numbers nobody could actually act on. Set `targetPrice` at **~15% below the current verified market/sold-comp price** — a realistic amount to negotiate off at an in-person card show for cash, not a number chosen to hit a target ROI. Net `expectedROI` should land around 8-12% as a result; if a specific pick can defensibly support more (a real, dated catalyst; a confirmed live listing below market), say so explicitly in the `source` field rather than just posting a bigger number. Never phrase `source` in a way that implies a specific findable listing exists at the target price — say "target: ~15% off current market via in-person negotiation," not "buy: [listing]."
